...
export MAIL_DOMAIN=$(grep ^mail_domain setting.log|sed "s/.*://")
...
if [ ${yn,,} = "y" ]; then
	...
	podman build -f Dockerfile-postfix-master -t postfix-master:latest --build-arg SSL_DOMAIN=$SSL_DOMAIN --build-arg MAIL_DOMAIN=$MAIL_DOMAIN
...
if [ ${yn,,} = "y" ]; then
	...
	podman build -f Dockerfile-postfix-slave -t postfix-slave:latest --build-arg SSL_DOMAIN=$replSSL_DOMAIN --build-arg MAIL_DOMAIN=$MAIL_DOMAIN
fi
