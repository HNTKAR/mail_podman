...
RUN dnf update -y && \
	dnf install -y rsyslog dovecot postfix && \
	dnf clean all

EXPOSE 25 465 587 993 995
ARG SSL_DOMAIN="mail.example.com"
ARG virtual_DOMAIN="mail.example.com,mail.example.org"
ARG alias_DOMAIN="example.com,example.org"
